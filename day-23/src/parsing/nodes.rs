    ...
    for (y, row) in grid.iter().enumerate() {
        for (x, tile) in row.iter().enumerate() {
            ...
            if is_node {
                ...
                if y > 0 {
                    if let Tile::Arrow(Direction::Up) = grid[y - 1][x] {
                        exits.push(Direction::Up);
                    }
                }
                if y < grid.len() - 1 {
                    if let Tile::Arrow(Direction::Down) = grid[y + 1][x] {
                        exits.push(Direction::Down);
                    }
                }
                if x > 0 {
                    if let Tile::Arrow(Direction::Left) = grid[y][x - 1] {
                        exits.push(Direction::Left);
                    }
                }
                if x < row.len() - 1 {
                    if let Tile::Arrow(Direction::Right) = grid[y][x + 1] {
